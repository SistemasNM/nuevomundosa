...
#Region "METODOS Y FUNCIONES"
    ...
    End Function
        ...
        'CAMBIO DG INI
        Function ConsultarFactoringRangoFechas(ByVal strFechIni As String, ByVal strFechFin As String, ByVal strEmpresa As String) As DataTable
            _objConexion = New AccesoDatosSQLServer(GeneradorCadenaConexion.enmBasesDatos.Intranet)
            Try
                Dim objParametros() As Object = {"vch_FechIni", strFechIni, _
                                                "vch_FechFin", strFechFin, _
                                                 "chr_Empresa", strEmpresa}
                Return _objConexion.ObtenerDataTable("usp_Listar_Factoring_Por_Fecha", objParametros)
            Catch ex As Exception
                _objError = ex
            Finally
                _objConexion = Nothing
            End Try
        End Function
        Function ConsultarDocumentosFactoring(ByVal strFactoring As String, ByVal strEmpresa As String) As DataTable
            _objConexion = New AccesoDatosSQLServer(GeneradorCadenaConexion.enmBasesDatos.Intranet)
            Try
                Dim objParametros() As Object = {"vch_Factoring", strFactoring, _
                                                 "chr_Empresa", strEmpresa}
                Return _objConexion.ObtenerDataTable("usp_Listar_Documentos_De_Factoring", objParametros)
            Catch ex As Exception
                _objError = ex
            Finally
                _objConexion = Nothing
            End Try
        End Function
        Function ActualizarEstadoFactoring(ByVal dt As DataTable, ByVal sUsuario As String) As DataTable
            _objConexion = New AccesoDatosSQLServer(GeneradorCadenaConexion.enmBasesDatos.Intranet)
            Try
                Dim clsUtilitario As New NM_General.Util
                Dim objParametros() As Object = {"vch_Usuario", sUsuario, _
                                                 "pvch_Dt", clsUtilitario.GeneraXml(dt)}
                Return _objConexion.ObtenerDataTable("usp_Actualizar_Factoring_Estado_Enviado", objParametros)
            Catch ex As Exception
                _objError = ex
            Finally
                _objConexion = Nothing
