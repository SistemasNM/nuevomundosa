...
Public Class NM_Parametros
  ...
  ''' <summary>
  ''' Adicionar una condicion Where a la consulta
  ''' </summary>
  ''' <value></value>
  ''' <returns></returns>
  ''' <remarks>vch_CodEmpresa = "01"</remarks>
...
#Region "DATOS"
  ...
  Public Function ListarDatosWhere() As DataTable
    Dim lobjCon As NM.AccesoDatos.AccesoDatosSQLServer
    Dim ldtRes As DataTable
    Try
      Dim lstrParametros() As String = {"chr_CodigoTabla", CodigoTabla, "vch_Where", Where}
      lobjCon = New NM.AccesoDatos.AccesoDatosSQLServer(NM.AccesoDatos.GeneradorCadenaConexion.enmBasesDatos.Produccion)
      ldtRes = lobjCon.ObtenerDataTable("usp_ADM_TablaMaestraColumnaDatoWhere_Listar", lstrParametros)
    Catch ex As Exception
      ldtRes = Nothing
    Finally
      lobjCon = Nothing
