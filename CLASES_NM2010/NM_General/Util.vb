...
#Region " Definicion de Metodos "
    ...
    Function EncodeString(ByVal Texto As String) As String
        Dim tramaCar() As String = {"—", "¡", "¢", "£", "¤", "¥", "¦", "§", "¨", "©", "ª", "«", "¬", "®", "¯", "°", "±", "²", "³", _
                                "´", "µ", "¶", "·", "¸", "¹", "º", "»", "¼", "½", "¾", "¿", "À", "Á", "Â", "Ã", "Ä", "Å", "Æ", "Ç", _
                                "È", "É", "Ê", "Ë", "Ì", "Í", "Î", "Ï", "Ð", "Ñ", "Ò", "Ó", "Ô", "Õ", "Ö", "×", "Ø", "Ù", "Ú", "Û", _
                                "Ü", "Ý", "Þ", "ß", "à", "á", "â", "ã", "ä", "å", "æ", "ç", "è", "é", "ê", "ë", "ì", "í", "î", "ï", _
    ...
    Function CheckHora(ByVal sHora As String) As Boolean
        If Trim(sHora) = "" Then
            Return False
        End If
        If Val(sHora.Chars(0).ToString) >= 0 AndAlso Val(sHora.Chars(0).ToString) <= 2 Then Return False
        If Val(sHora.Chars(1).ToString) >= 0 AndAlso Val(sHora.Chars(1).ToString) <= 9 Then Return False
        If sHora.Chars(2).ToString <> ":" Then Return False
        If Val(sHora.Chars(3).ToString) >= 0 AndAlso Val(sHora.Chars(3).ToString) <= 5 Then Return False
        If Val(sHora.Chars(4).ToString) >= 0 AndAlso Val(sHora.Chars(4).ToString) <= 9 Then Return False
    ...
    Public Function FormatFecha_Gandi(ByVal sFecha As String) As String
        ...
        strFormato = Registry.LocalMachine.OpenSubKey("Software\NuevoMundo\GANDI").GetValue("FormatDate", "")
        Return Format(CDate(sFecha), strFormato)
    End Function

    Public Function FormatFechaHora_GANDI(ByVal pFecha As DateTime) As String
        Dim strFormato As String
        strFormato = Registry.LocalMachine.OpenSubKey("Software\NuevoMundo\GANDI").GetValue("FormatDateTime", "")
        Return Format(pFecha, strFormato)
    End Function

    Public Function FormatFechaHora(ByVal pFecha As DateTime) As String
        Dim strFormato As String
        strFormato = Registry.LocalMachine.OpenSubKey("Software\NuevoMundo").GetValue("FormatDateTime", "")
        Return Format(pFecha, strFormato)
    End Function
    ...
    Public Overloads Function FormatFecha(ByVal sFecha As String) As String
        If Trim(sFecha) = "" Or Not IsDate(sFecha) Then Return ""
        Dim strFormato As String
        strFormato = Registry.LocalMachine.OpenSubKey("Software\NuevoMundo").GetValue("FormatDate", "")
        Return Format(CDate(sFecha), strFormato)
    End Function
    ...
    Public Overloads Function FormatFecha(ByVal pFecha As DateTime) As String
        ...
        Return Format(pFecha, strFormato)
    End Function
    Public Shared Function Encrypt(pstrEncryptar As String, Optional ByVal pstrEncryptionKey As String = "NM_PROY_WEB") As String
        Dim EncryptionKey As String = pstrEncryptionKey
        Dim clearBytes As Byte() = Encoding.Unicode.GetBytes(pstrEncryptar)
        Using encryptor As Aes = Aes.Create()
            ...
            Using ms As New MemoryStream()
                Using cs As New CryptoStream(ms, encryptor.CreateEncryptor(), CryptoStreamMode.Write)
                    ...
                    cs.Close()
                End Using
                pstrEncryptar = Convert.ToBase64String(ms.ToArray())
            End Using
        End Using
        Return pstrEncryptar
    End Function

    Public Shared Function Decrypt(pstrDesencryptar As String, Optional ByVal pstrEncryptionKey As String = "NM_PROY_WEB") As String
        Dim EncryptionKey As String = pstrEncryptionKey
        pstrDesencryptar = pstrDesencryptar.Replace(" ", "+")
        Dim cipherBytes As Byte() = Convert.FromBase64String(pstrDesencryptar)
        Using encryptor As Aes = Aes.Create()
            Dim pdb As New Rfc2898DeriveBytes(EncryptionKey, New Byte() {&H49, &H76, &H61, &H6E, &H20, &H4D, _
            ...
            Using ms As New MemoryStream()
                Using cs As New CryptoStream(ms, encryptor.CreateDecryptor(), CryptoStreamMode.Write)
                    ...
                    cs.Close()
                End Using
                pstrDesencryptar = Encoding.Unicode.GetString(ms.ToArray())
            End Using
        End Using
        Return pstrDesencryptar
    End Function
#End Region
